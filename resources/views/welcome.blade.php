            ...
            <a href="{{ route('post.show', $topPost) }}" class="block relative rounded-2xl overflow-hidden mb-10 group h-[480px]">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-8 flex flex-col gap-3">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-medium px-3 py-1 rounded-full bg-white/20 text-white backdrop-blur-sm">{{ $topPost->category->name ?? 'Uncategorized' }}</span>
                        <span class="text-xs text-gray-400">{{ $topPost->created_at->diffForHumans() }}</span>
                        <span class="text-xs px-2 py-0.5 rounded-full bg-green-900/50 text-green-400">{{ $topPost->status }}</span>
                    ...
                    <p class="text-gray-300 text-sm line-clamp-2 max-w-2xl">{{ $topPost->description }}</p>
